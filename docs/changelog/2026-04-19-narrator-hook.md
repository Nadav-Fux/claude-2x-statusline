...
The real narrator lives somewhere more useful: **above the user's next prompt**, injected via the `SessionStart` hook — the same mechanism plugins use to surface memory into the response stream.
...
ה-narrator האמיתי חי במקום שיותר שימושי: **מעל הפרומפט הבא של המשתמש**, מוזרק דרך hook של `SessionStart` — אותו מנגנון שבו תוספים טוענים זיכרון לתוך זרם התגובה.
