...
// Session suffix — when set, state is stored per-session to avoid
// cross-session cost contamination from parallel Claude Code instances.
let _sessionSuffix = '';

function setSessionId(sessionId) {
  if (sessionId && sessionId.length >= 4) {
    _sessionSuffix = sessionId.substring(0, 8);
  } else {
    _sessionSuffix = '';
  }
}
...
function statePath() {
  return _sessionSuffix
    ? path.join(CLAUDE_DIR, `statusline-state-${_sessionSuffix}.json`)
    : STATE_PATH;
}

function tmpPath() {
  return _sessionSuffix
    ? path.join(CLAUDE_DIR, `statusline-state-${_sessionSuffix}.json.tmp`)
    : TMP_PATH;
}
...
// Most recent session-scoped sample (or null). Lets the narrator fall back to
// token data when hook stdin omits it — parity with the Python path.
