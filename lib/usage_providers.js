...
// bottom row (see formatCombinedGlmCopilotRow) whenever both are enabled.
// Kept in sync with lib/usage_providers.py EXTERNAL_PROVIDER_ORDER and
// engines/python-engine.py + lib/onboarding.py's copies.
const EXTERNAL_PROVIDER_ORDER = ['codex', 'droid', 'antigravity', 'glm', 'copilot'];
...
// is not guaranteed to be the same for every account/plan. parseGlmQuotaResponse
// therefore classifies each limit by how soon it ACTUALLY resets
// (nextResetTime - now) instead of trusting `type`. See glmWindowClassification.
const GLM_SHORT_WINDOW_MAX_SECONDS = 6 * 3600; // <= ~6h  -> '5h' / five_hour slot
const GLM_LONG_WINDOW_MIN_SECONDS = 2 * 86400; // >= ~2d  -> 'mo' / weekly slot
const GLM_DAY_WINDOW_MAX_SECONDS = 30 * 3600; // in-between band, day-ish half -> 'day'
// A live `codex app-server` snapshot (source: 'app-server', written by the
// detached Python refresher) stays authoritative this long. Mirrors the Python
// CODEX_LIVE_TTL. This twin is cache-read-only for the live record: it renders
// whatever the refresher wrote and never spawns app-server itself.
const CODEX_LIVE_TTL = 120;
// The two-pool Antigravity quota-summary cache (5h + weekly per pool) is written
// by the Python background refresher; this twin only reads it. Mirrors
// ANTIGRAVITY_CACHE_TTL in usage_providers.py.
const ANTIGRAVITY_CACHE_TTL = 120;
// A stale quota-summary still beats the CLI's 5h-only view; only past this
// horizon does the render drop to the CLI fallback.
const ANTIGRAVITY_SUMMARY_MAX_AGE = 6 * 3600;
...
// Classify one GLM quota window by how soon it resets (seconds from now), NOT
// by GLM's `type` field -- see the GLM_SHORT_WINDOW_MAX_SECONDS comment above
// for why `type` can't be trusted. Returns [slot, label]: slot is 'five_hour'
// (genuinely short, <=~6h) or 'weekly' (genuinely long, >=~2d); the ~6h-2d gap
// in between still resolves to one of the two slots but with an honest
// 'day'/'wk' label instead of forcing '5h'/'mo' onto a window that is
// neither. [null, null] means the interval is unknown (e.g. no nextResetTime
// yet) and the caller decides a fallback placement.
function glmWindowClassification(intervalSeconds) {
  if (intervalSeconds == null) return [null, null];
  if (intervalSeconds <= GLM_SHORT_WINDOW_MAX_SECONDS) return ['five_hour', '5h'];
  if (intervalSeconds >= GLM_LONG_WINDOW_MIN_SECONDS) return ['weekly', 'mo'];
  if (intervalSeconds <= GLM_DAY_WINDOW_MAX_SECONDS) return ['five_hour', 'day'];
  return ['weekly', 'wk'];
}
...
// Reset text for a compact row: one reset per metric, not just the soonest.
//
// A compact row can carry two metrics on very different cadences (GLM's 5h +
// monthly meters share one row -- see formatCombinedGlmCopilotRow).
// soonestResetText picks a single winner, which would permanently hide the
// longer meter's own reset date behind the short meter's sooner clock, since
// the short one always resets first. Showing every metric's own reset instead
// (each in its own style via resetStyleForLabel) keeps both dates/clocks
// visible, in metric order, joined with the same dim separator used between
// the metrics themselves.
function compactMetricsResetText(metrics, nowSec, formatDuration, formatClock) {
  const pieces = metrics
    .map(metric => resetDisplay(metric.resets_at, metric.label, nowSec, formatDuration, formatClock))
    .filter(Boolean);
  return pieces.join(' · ');
}
...
function formatProviderRowText(row) {
  ...
  if (row.display === 'compact') {
    ...
    const metricParts = parts.filter(p => p && p.kind === 'metric');
    // New-style rows carry a resetText on EVERY metric part (possibly '' for
    // a metric with no reset), so each meter shows ITS OWN reset inline
    // instead of bunching every meter's reset at the row's end. Older
    // hand-built compact rows (e.g. antigravityDualRows, which sets one
    // soonest-based resetText on the ROW instead) never set that key on
    // their metric parts, so they keep the legacy trailing-reset render.
    const hasPerMetricReset = metricParts.some(p => Object.prototype.hasOwnProperty.call(p, 'resetText'));
    let metrics, reset;
    if (hasPerMetricReset) {
      metrics = metricParts.map(p => `${p.label} ${p.pct}%` + (p.resetText ? ` ${p.resetText}` : ''));
      reset = '';
    } else {
      metrics = metricParts.map(p => `${p.label} ${p.pct}%`);
      reset = row.resetText ? ` ${row.resetText}` : '';
    }
  ...
  if (display === 'compact') {
    ...
    for (const metric of metrics) {
      parts.push({
        ...
        resetText: resetDisplay(metric.resets_at, metric.label, nowSec, formatDuration, formatClock),
      });
    }
    ...
    const row = {
      ...
      display,
      parts,
      // Each metric above now carries its own resetText, so a two-cadence
      // compact row (GLM's 5h + monthly meters) shows each reset right after
      // its own meter instead of bunching both at the row's end. Kept (empty)
      // only for shape parity with non-compact rows.
      resetText: '',
      stale,
      staleText: stale ? ' \u00b7stale' : '',
    };
...
// Merge GLM + Copilot into ONE bottom-slot row (both share the cockpit's last
// row instead of two separate lines). Mirrors the python twin
// format_combined_glm_copilot_row exactly.
//
...
// + plan chip + its one metric, e.g. "2152 left 28%" \u2014 no bar) and the two are
// joined with the same dim ' \u00b7 ' separator already used between metrics
...
// Returns null when neither side has data.
function formatCombinedGlmCopilotRow(glmRecord, copilotRecord, nowSec = Date.now() / 1000, options = {}) {
  const glmRow = glmRecord && typeof glmRecord === 'object' && !Array.isArray(glmRecord)
    ? formatProviderRowParts(glmRecord, nowSec, options)
    : null;
  const copilotRow = copilotRecord && typeof copilotRecord === 'object' && !Array.isArray(copilotRecord)
    ? formatProviderRowParts(copilotRecord, nowSec, options)
    : null;

  if (!(glmRow && copilotRow)) return glmRow || copilotRow || null;

  const copilotCompactSource = { ...copilotRecord, display: 'compact' };
  const copilotCompactRow = formatProviderRowParts(copilotCompactSource, nowSec, options) || copilotRow;

  const segments = [glmRow, copilotCompactRow];
  const row = {
    label: glmRow.label || '',
    display: 'combined',
    segments,
    stale: Boolean(glmRow.stale || copilotCompactRow.stale),
    staleText: glmRow.staleText || copilotCompactRow.staleText || '',
  };
...
function readCachedExternalUsage(config = {}) {
  try {
    ...
    const records = [];
    for (const provider of EXTERNAL_PROVIDER_ORDER) {
...
// by collectExternalUsage, or a full config carrying external_providers.codex.
// Default is falsy: Codex renders a single row for the SELECTED plan.
function codexShowAllPlans(config) {
  if (!config || typeof config !== 'object') return false;
  let flag = config.show_all_plans;
  if (!flag) {
    const external = config.external_providers;
    if (external && typeof external === 'object' && external.codex && typeof external.codex === 'object') {
      flag = external.codex.show_all_plans;
    }
  }
  return Boolean(flag);
}
...
// A codex window whose reset time has already passed is provably stale. Codex
// usage comes only from local rollout logs, so a snapshot freezes between
// sessions. Once resets_at is in the past, the window has demonstrably reset,
// and the absence of any newer rollout for this plan proves zero recorded
// usage since — so render 0%, not a frozen (often 100%) reading, instead of
// misleading the owner into thinking the window is still hot.
function healCodexWindow(window, now) {
  if (!window || typeof window !== 'object' || Array.isArray(window)) return window;
  const resetsAt = window.resets_at;
  if (typeof resetsAt !== 'number' || !Number.isFinite(resetsAt)) return window;
  if (resetsAt > now) return window;
  const healed = { used_pct: 0, resets_at: null };
  if (window.label) healed.label = window.label;
  return healed;
}
...
// readCachedRecord returns), so a cached record read up to LOCAL_CACHE_TTL
// seconds later still renders healed. Also walks all_plans so every per-plan
// row gets the same treatment. Idempotent and cheap, so calling it twice on
// the same record is harmless.
function healCodexRecord(record, now = null) {
  if (!record || typeof record !== 'object' || Array.isArray(record)) return record;
  const nowSec = now == null ? Date.now() / 1000 : now;
  const healed = { ...record };
  for (const key of ['five_hour', 'weekly']) {
    if (key in healed) healed[key] = healCodexWindow(healed[key], nowSec);
  }
  if (Array.isArray(healed.all_plans)) {
    healed.all_plans = healed.all_plans.map(r => healCodexRecord(r, nowSec));
  }
  return healed;
}

// The cached codex record + its age, but only when it is a live app-server
// snapshot (source: 'app-server'). The rollout fallback and the Python live
// refresher share one cache file; the source tag tells a live snapshot apart
// from a frozen rollout snapshot. Returns [record, ageSeconds] or [null, null].
function readCodexLiveCache() {
  const file = cachePath('codex');
  try {
    const age = Math.max(0, Math.trunc(Date.now() / 1000 - fs.statSync(file).mtimeMs / 1000));
    const data = readJson(file);
    const record = data && typeof data.record === 'object' && !Array.isArray(data.record) ? data.record : null;
    if (!record || record.source !== 'app-server' || record.available !== true) return [null, null];
    return [record, age];
...
function getCodexUsage(config = {}) {
  try {
    // Prefer a fresh LIVE app-server snapshot for the currently authenticated
    // account. A pin selecting a different plan than the live account, or
    // show_all_plans (which needs every plan the rollouts saw), falls back to
    // the rollout scan. This twin only READS the cache — the detached Python
    // refresher is what writes the live snapshot; a stale live record (older
    // than CODEX_LIVE_TTL) yields to the twin's own fresh rollout scan.
    const [live, liveAge] = readCodexLiveCache();
    const pin = codexPlanPin(config);
    const showAll = codexShowAllPlans(config);
    const livePlan = live ? String(live.plan || '').trim().toLowerCase() : '';
    const pinMatchesLive = !pin || (!!livePlan && pin === livePlan);
    if (!showAll && live && pinMatchesLive && liveAge != null && liveAge < CODEX_LIVE_TTL) {
      return healCodexRecord({ ...live, stale_seconds: liveAge });
    }

    // Rollout fallback (self-healing, exactly as today). A live snapshot in the
    // shared cache is not a rollout answer, so never let it satisfy the fallback
    // (a pin/show_all render must not render the wrong account/plan).
    const cached = readCachedRecord('codex', LOCAL_CACHE_TTL);
    if (cached && cached.source !== 'app-server') return healCodexRecord(cached);
    ...
    // Opt-in: only attach all_plans (and thus render one row per plan) when the
    // owner explicitly asks for it. Default renders a single row for the
    // SELECTED plan (pin > newest paid > newest overall), matching one Codex
    // subscription per row.
    const record = healCodexRecord(
      showAll
        ? { ...selected, all_plans: codexAllPlans(ordered, config) }
        : { ...selected },
    );
    // Never clobber a still-fresh live snapshot with a rollout record: preserve
    // it so the next render keeps comparing against the real account.
    if (!live || (liveAge != null && liveAge >= CODEX_LIVE_TTL)) writeCachedRecord('codex', record);
    return record;
  } catch { return unavailable('codex'); }
}

function parseGlmQuotaResponse(data, staleSeconds = null, nowSec = null) {
  let body = data;
  ...
  const now = nowSec == null ? Date.now() / 1000 : numberOrZero(nowSec);
  ...
  // earns comes first; a limit that couldn't be classified (no nextResetTime
  // yet, e.g. a window the API hasn't attached a fresh countdown to) or that
  // collides with an already-filled slot falls back to whichever meter is
  // still empty, so both meters keep filling even when a limit's cadence
  // can't be read from this snapshot alone.
  const slots = { five_hour: null, weekly: null };
  const leftovers = [];
  for (const item of limits) {
    if (!item || typeof item !== 'object') continue;
    let reset = null;
    if (item.nextResetTime != null) {
      const ms = Number(item.nextResetTime);
      if (Number.isFinite(ms)) reset = Math.round(ms / 1000);
    }
    const interval = reset == null ? null : reset - now;
    const [slot, label] = glmWindowClassification(interval);
    const window = usageWindow(item.percentage, reset, label);
    if (!window) continue;
    if (slot && !slots[slot]) slots[slot] = window;
    else leftovers.push(window);
  }

  for (const window of leftovers) {
    if (!slots.five_hour) {
      if (!window.label) window.label = '5h';
      slots.five_hour = window;
    } else if (!slots.weekly) {
      if (!window.label) window.label = 'mo';
      slots.weekly = window;
    }
  }

  const fiveHour = slots.five_hour;
  ...
  return {
    ...
    display: 'compact',
    metrics: [
      fiveHour ? { label: fiveHour.label || '5h', used_pct: fiveHour.used_pct, resets_at: fiveHour.resets_at } : null,
      weekly ? { label: weekly.label || 'mo', used_pct: weekly.used_pct, resets_at: weekly.resets_at } : null,
    ].filter(Boolean),
...
const ANTIGRAVITY_POOL_ORDER = { Gemini: 0, 'Claude+GPT': 1 };

// Classify one `antigravity-usage` models[] entry into Antigravity's real quota
// pool.
//
...
// by label/modelId keyword: "gemini", "flash", or "pro" land in the Gemini pool
// (the flash/pro terms keep old bare-label lineups, e.g. a bare "Flash" or
// "Pro" label with no "Gemini" prefix, in the right pool); everything else
// falls into the Claude+GPT pool.
function antigravityPool(model) {
  ...
  if (hint.includes('gemini') || hint.includes('flash') || hint.includes('pro')) return 'Gemini';
  return 'Claude+GPT';
}

// Map an `antigravity-usage quota --json` snapshot into Antigravity's two real
// quota pools (models[].remainingPercentage is a 0..1 fraction).
//
// models[].remainingPercentage measures only the 5-hour dimension of each pool
// (the CLI's raw quotaInfo carries just remainingFraction + resetTime, and
// every resetTime observed is a ~5h boundary); weekly data is not obtainable
// from this CLI, so these metrics stay 5h-only and no weekly figure is
// fabricated.
//
// Within a pool, used_pct is the MAX used_pct across member models (the worst
// case — equivalent to the pool's min remaining, matching the CLI's own --all
// pooling behavior); resets_at is that member's resetTime, or the earliest one
// among members tied on used_pct. Order: Gemini first, then Claude+GPT.
function mapAntigravitySnapshot(snapshot) {
  ...
  for (const m of models) {
    ...
    const existing = groups[pool];
    if (
      !existing ||
      usedPct > existing.used_pct ||
      (usedPct === existing.used_pct && resetsAt != null && (existing.resets_at == null || resetsAt < existing.resets_at))
    ) {
      groups[pool] = { label: pool, used_pct: usedPct, resets_at: resetsAt };
    }
  }
  const names = Object.keys(groups);
  if (!names.length) return null;
  names.sort((a, b) => (ANTIGRAVITY_POOL_ORDER[a] ?? 99) - (ANTIGRAVITY_POOL_ORDER[b] ?? 99));
  return names.map(name => groups[name]);
}

// Antigravity quota. Prefers the two-pool quota-summary cache (5h + weekly per
// pool, written by the Python background refresher — this twin stays
// cache-read-only for that route), then falls back to the `antigravity-usage`
// CLI 5h-only compact path. We cache BOTH CLI hits and misses for GLM_CACHE_TTL
// so a logged-out machine never re-spawns the CLI on every render.
function getAntigravityUsage(config = {}) {
  try {
    // 1) Two-pool quota-summary cache wins — even stale (up to the horizon),
    // because 5h+weekly per pool beats the CLI's 5h-only view. The Python
    // refresher renews it in the background; staleness stays honest via
    // stale_seconds. Mirrors ANTIGRAVITY_SUMMARY_MAX_AGE in usage_providers.py.
    const [data, age] = readFreshCache('antigravity', ANTIGRAVITY_SUMMARY_MAX_AGE);
    const summaryRecord = data && typeof data.record === 'object' && !Array.isArray(data.record) ? data.record : null;
    if (isAvailableRecord(summaryRecord) && summaryRecord.source === 'quota-summary') {
      return { ...summaryRecord, stale_seconds: Math.max(0, age || 0) };
    }

    // 2) Existing CLI 5h-only compact path (own cache file — must NEVER clobber
    // the richer quota-summary record in the main cache).
    const cached = readCachedRecord('antigravity-cli', GLM_CACHE_TTL);
    if (cached) return cached;
    ...
    const record = metrics
      ...
      : unavailable('antigravity');
    writeJson(cachePath('antigravity-cli'), { cached_at: Date.now() / 1000, record });
    ...
    // downgrade a usable summary record. Mirrors the python guard.
    try {
      const main = readJson(cachePath('antigravity'));
      const mainRecord = main && typeof main.record === 'object' && !Array.isArray(main.record) ? main.record : null;
      const mainAge = main && Number.isFinite(Number(main.cached_at)) ? Date.now() / 1000 - Number(main.cached_at) : null;
      const summaryAlive = mainRecord && mainRecord.source === 'quota-summary'
        && mainAge != null && mainAge < ANTIGRAVITY_SUMMARY_MAX_AGE;
      if (!summaryAlive) {
