...
// is produced out-of-band by ~/.claude/copilot-credits-refresh.sh (GitHub billing
// API). This twin only reads the cached record so the node engine renders the same
// row; the stale-while-revalidate refresh is python/script-driven (see the python
// get_copilot_usage), so node never spawns the refresher itself.
