...
function soonestResetText(metrics, nowSec, formatDuration) {
  let soonest = null;
  for (const metric of metrics) {
    const reset = Number(metric.resets_at);
    if (!Number.isFinite(reset) || reset <= nowSec) continue;
    if (soonest == null || reset < soonest) soonest = reset;
  }
  return soonest == null ? '' : resetCountdown(soonest, nowSec, formatDuration);
}
...
function formatProviderRowParts(record, nowSec = Date.now() / 1000, options = {}) {
  ...
  if (display === 'compact') {
    ...
    const row = {
      ...
      display,
      parts,
      resetText: soonestResetText(metrics, nowSec, formatDuration),
      stale,
