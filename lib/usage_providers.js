...
function getCodexUsage(config = {}) {
  try {
    ...
    const record = selectCodexSnapshot(codexRolloutSnapshots(), config);
    if (!record) return unavailable('codex');
...
function antigravityCreditsMetric(credits) {
  ...
  return { label: 'cr', used_pct: Math.max(0, Math.min(100, Math.round(used * 100))), resets_at: null };
}
