...
def generate(obs: "Observation", memory: dict) -> Optional[str]:
    ...
    user_payload = {
        ...
        "rules_engine_pick": None,  # Filled by engine before calling
...
def generate_with_rules_context(
    ...
    # We patch the generate fn temporarily by mutating memory's current narratives context
    # Actually we just call generate, but we need to inject rules_pick_text.
    # Since generate() builds its own payload, we do a small override here.
    api_key = os.environ.get("ANTHROPIC_API_KEY", "")
    if not api_key:
        return None
